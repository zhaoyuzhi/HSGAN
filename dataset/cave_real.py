...
class HS_multiscale_DSet(Dataset):
    def __init__(self, opt):
        self.opt = opt
        # build image list
        real_data_path = os.path.join(opt.baseroot_train, 'real_data')
        self.imglist = utils.get_bmps(real_data_path)
    ...
    def __getitem__(self, index):
        ...
        # read a rgb image
        rgbpath = os.path.join(self.opt.baseroot_train, 'generated_rgb_realworld', object_name + '.png')
...
class HS_multiscale_ValDSet(Dataset):
    def __init__(self, opt):
        self.opt = opt
        # build image list
        real_data_path = os.path.join(opt.baseroot_val, 'real_data')
        self.imglist = utils.get_bmps(real_data_path)
    ...
    def __getitem__(self, index):
        ...
        # read a rgb image
        rgbpath = os.path.join(self.opt.baseroot_val, 'generated_rgb_realworld', object_name + '.png')
        img = cv2.imread(rgbpath)
...
if __name__ == "__main__":
    ...
    path = 'F:\\dataset, task related\\Hyperspectral Imaging\\CAVE\\generated_rgb\\train'
