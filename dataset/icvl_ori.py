...
class HS_multiscale_DSet(Dataset):
    def __init__(self, opt):
        self.opt = opt
        # build image list
        self.imglist = utils.get_mats(opt.baseroot_train)
    
    def __getitem__(self, index):

        # read an image
        filepath = self.imglist[index]
        jointfile = h5py.File(filepath)
...
class HS_multiscale_ValDSet(Dataset):
    def __init__(self, opt):
        self.opt = opt
        # build image list
        self.imglist = utils.get_mats(opt.baseroot_val)
    ...
    def __getitem__(self, index):
        ...
        rgb = jointfile['rgb']
        rgb = np.transpose(rgb, (2, 1, 0))
        rgb = rgb[(rgb.shape[0] - spectral.shape[0]):, (rgb.shape[1] - spectral.shape[1]):, :]
        #print(rgb.shape, spectral.shape), e.g., (1392, 1300, 3) (1392, 1300, 31)
...
if __name__ == "__main__":
    ...
    def read_one_mat(in_path):
        ...
        for i in range(31):
            ...
            img = np.rot90(img)
            newdata[:, :, i] = img
        print('spectral.shape', newdata.shape, newdata.dtype)
        rgb = spectral['rgb']
    ...
    imgpath = 'E:\\dataset, task related\\Hyperspectral Imaging\\ICVL\\train\\spectral\\Lehavim_0910-1627.mat'
    imgpath = 'E:\\dataset, task related\\Hyperspectral Imaging\\ICVL\\val\\spectral\\bguCAMP_0514-1718.mat'
